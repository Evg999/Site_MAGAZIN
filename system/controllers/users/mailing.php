...
if (!empty($_POST['email'])) {
    if ($amount > 0) {
        //редирект
        header('location: https://mag24-shop.ru/index.php' . '?inDatabase');
    } else {
        //создаем новую строчку в таблице
        mysqli_query($connect->getConnection(), "INSERT INTO mailings(email) VALUES('$email') ");
        header('location: https://mag24-shop.ru/index.php' . '?success=1');
    }
} else {
    //редирект
    header('location: https://mag24-shop.ru/index.php' . '?wrong=1');
}
